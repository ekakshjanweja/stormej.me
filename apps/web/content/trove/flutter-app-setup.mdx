...
The architecture uses `lib/src/core`, feature modules under
`lib/src/modules`, infrastructure under `lib/src/services`, a normal
`ProviderScope`, and a one-shot startup provider. The API client uses Dio for
JSON, multipart, and streamed responses.
</Callout>
